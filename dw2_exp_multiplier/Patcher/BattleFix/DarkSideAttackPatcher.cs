...
namespace dw2_exp_multiplier.Patcher.BattleFix
{
    public class DarkSideAttackPatcher : IPatcher
    {
        ...
        private void patchBtyesJAP(ref FileStream fs)
        {
            ...
            Buffer.BlockCopy(patchedPattern, 0, data, 0xB700, patchedPattern.Length);

            patchedPattern = new byte[]
            {
                0x4F, 0x03, 0x43, 0x90, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x62, 0x30,
                0x04, 0x00, 0x40, 0x14, 0x14, 0x00, 0x83, 0x86, 0x14, 0x00, 0x82, 0x96,
                0x4C, 0xB8, 0x01, 0x08, 0x23, 0x90, 0x64, 0x00, 0x16, 0x00, 0x82, 0x96,
                0x16, 0x00, 0x83, 0x86, 0x4C, 0xB8, 0x01, 0x08, 0x23, 0x90, 0x64, 0x00
            };
            Buffer.BlockCopy(patchedPattern, 0, data, 0x10F8C, patchedPattern.Length);
        ...
        private bool ValidateBytesJAP()
        {
            ...
            for (int i = 0, j = 0xB700; i < bytes.Length; i++)
            {
                if (bytes[i] != data[j + i])
                    return false;
            }

            for (int i = 0, j = 0x10F8C; i < 0x30; i++)
            {
                if (data[j + i] != 0)
                    return false;
            }
