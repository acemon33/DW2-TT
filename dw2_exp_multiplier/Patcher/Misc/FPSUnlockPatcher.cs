...
namespace dw2_exp_multiplier.Patcher.Misc
{
    public class FPSUnlockPatcher : IPatcher
    {
        ...
        {
            byte[] patchedPattern = { 0x3a, 0x27, 0x01, 0x08 };
            Buffer.BlockCopy(patchedPattern, 0, data, 0x13C7C, patchedPattern.Length);

            patchedPattern = new byte[] {
                0x05, 0x80, 0x02, 0x3C, 0xAB, 0xF6, 0x42, 0x34, 0x00, 0x00, 0x42, 0x90,
                0xF3, 0x00, 0x03, 0x34, 0x03, 0x00, 0x43, 0x14, 0xFC, 0x00, 0x03, 0x34,
                0x02, 0x80, 0x02, 0x3C, 0xB8, 0x34, 0x40, 0xAC, 0x04, 0x00, 0x43, 0x14,
                0x02, 0x80, 0x03, 0x3C, 0x40, 0x14, 0x02, 0x3C, 0x1B, 0x00, 0x42, 0x24,
                0xB8, 0x34, 0x62, 0xAC, 0x08, 0x00, 0xE0, 0x03, 0x00, 0x00, 0x00, 0x00
            };
            Buffer.BlockCopy(patchedPattern, 0, data, 0x3A4E8, patchedPattern.Length);
        ...
        private bool ValidateBytesUS()
        {
            ...
            for (int i = 0, j = 0x3A4E8; i < 0x3C; i++)
            {
                if (data[j + i] != 0)
                    return false;
            }
